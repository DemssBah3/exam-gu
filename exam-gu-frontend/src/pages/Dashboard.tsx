...
export const Dashboard = () => {
  ...
  // Récupérer les données en temps réel (seulement pour Admin)
  ...
  const activeSessions = sessions?.slice(0, 5).map(session => {
    ...
    const enrollmentCount = enrollments?.filter(e => e.sessionId === session.id).length || 0;
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2">
          ...
          {user?.role === 'STUDENT' && 'Tableau de bord étudiant'}
        </p>
      </div>
      ...
      {user?.role === 'ADMIN' && activeSessions && activeSessions.length > 0 && (
      ...
      {user?.role === 'ADMIN' && recentActivities && recentActivities.length > 0 && (
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {user?.role === 'TEACHER' && (
            <div className="p-4 border border-gray-200 rounded-lg">
              ...
              <p className="text-sm text-gray-600 mt-1">À venir prochainement</p>
            </div>
          )}
